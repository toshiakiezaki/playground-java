...
public class ViaCepClient {
    ...
    public Mono<PostalCode> findByCode(String code) {
        return client.get().uri(String.format("/ws/%s/%s", code, DEFAULT_TYPE)).accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(ViaCepData.class)
                .map(ViaCepData::toEntity);
    }
